...
$\bar{\delta}^{\,*}_n(\xi)$ без дополнительных аппроксимаций, где
\[
\bar{\delta}^{\,*}_n(\xi)
=
\frac{\bar{\delta}_n(\xi)}
{\int_{-\infty}^{\infty} f^2(x)\,dx}
=2\sqrt{3}\,\bar{\delta}_n(\xi).
\]

\begin{figure}[H]
\centering
\plotplaceholder[0.82\textwidth]{delta_bar_n30_100.png}
\caption{Графики нормированной ОИСКО $\bar{\delta}^{\,*}_{30}(\xi)$ и $\bar{\delta}^{\,*}_{100}(\xi)$, построенные по аналитической формуле для прямоугольного ядра.}
\end{figure}

При малых $\xi$ ошибка быстро растёт из-за дисперсионного члена
$\sim (n\xi)^{-1}$, а при больших $\xi$ кривая приближается к
нормированному пределу $1$.
